...
$.widget("bootstrap.bwizard", {
	...
	_refreshStep: function () {
		...
		if (this.lis) {
			this.lis.removeClass('current').attr('aria-selected', false).find('.label').removeClass('');
			if (o.activeIndex >= 0 && o.activeIndex <= this.lis.length - 1) {
				if (this.lis) {
					this.lis.eq(o.activeIndex).addClass('current').attr('aria-selected', true).find('.label').addClass('');
				}
			}
		}
